...
-- Схема 1: app_auth - Аутентификация и пользователи
...
CREATE TABLE app_auth.users (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- Note: This often references auth.users(id) in Supabase
...
-- Автоматическое создание пользователя в app_auth.users при регистрации
CREATE OR REPLACE FUNCTION app_auth.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO app_auth.users (id, email, full_name, role)
  VALUES (
    new.id,
    new.email,
...
-- Note: Requires permissions on auth.users which supabase admin has
CREATE OR REPLACE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE PROCEDURE app_auth.handle_new_user();

-- Схема 2: app_projects - Проекты и доски
...
-- Схема 3: app_tasks - Задачи и карточки
...
-- Таблица связей задач (для декомпозиции и зависимостей)
...
-- Схема 4: app_analytics - Аналитика и KPI

-- Таблица метрик команды
...
-- Схема 5: app_ai - AI конфигурации и промпты

-- Таблица AI ассистентов
...
-- Политика для задач: пользователи видят только задачи своей организации
...
-- Политика для редактирования: только назначенные или создатели
...
-- Политика для аналитики: только admin и team_lead
...
-- Алгоритм позиционирования карточек (Weight-based)
